% This is a change file for e-upTeX.
% Licensed under BSD 3-Clause.
%
% e-upTeX is developed by Japanese TeX Development Community,
% based on:
%   * pTeX by Sadayuki Tanaka and ASCII MEDIA WORKS
%   * upTeX by Takuji Tanaka
%   * e-pTeX by Hironori Kitagawa
%

% pTeX Copyright Notice & Change History:
% ==================================================================
...
% for Mathematicians II''(http://ks.ms.u-tokyo.ac.jp/ (in Japanese),
% 2007 Winter), and it is licensed under modified BSD (same as pTeX,
% ptexenc).
%
...
@x [2.??] l.586 - pTeX: 16-bit charcode -> upTeX: 24-bit charcode
...
@x [13.202] l.4056 - pTeX: disp_node
...
@x [15.209] l.4272 - pTeX: def_jfont, def_tfont, set_auto_spacing. upTeX: set_enable_cjk_token
...
@x (cont.) adjustment for partoken.ch is done here too.
...
@x [20.289] l.6387 - pTeX + upTeX: cs_token_flag
...
@x e-pTeX: \currentspacingmode, \currentxspacingmode. e-upTeX: \currentcjktoken.
...
@x [26.424] l.8516 - pTeX: Fetch an item ...: disp_node + e-pTeX: node char
...
@x [26.424] e-pTeX: \lastnodesubtype
...
@x [27.464] l.9475 - pTeX + upTeX: str_toks
...
@x [28.487] l.9852 - pTeX: if directions (leave room for three e-TeX codes)
...
@x [28.487] l.9887 - pTeX: if directions
...
@x [28.488] l.9907 - pTeX: if directions
...
% (texqa:55735)
% 根号を表現するメモリ領域 (left_delimiter) が
% 和文文字の漢字コードを格納する場所と同じで上書きしてしまうのを回避．
% 今度は right_delimiter が衝突するが，fraction_noad でのみしか使われないので無問題か．
% % 昔の pTeX では left_delimiter の中身は「右」4バイトに，
% % 一方 math_kcode は「左」4バイトに格納されるので衝突は起こらなかった．
% % 現在の pTeX では（e-pTeX, upTeX も同様）left_delimiter の中身は
% % 8バイト全てを使って格納されるので衝突する．
% % texmfmem.h 内で定義されている memory_word を参照．
...
@x [34.687] accent with japanese char (forum:422)
...
@x [49.1210] l.23361 - pTeX: set_auto_spacing + upTeX: set_enable_cjk_token
...
@x [49.1256] l.24163 - pTeX: def_jfont, def_tfont
def_font: new_font(a);
@y
...
@x [49.1291] l.24467 - pTeX: show_mode (leave room for three e-TeX codes)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% pTeX enhancements with adjustments:
%   * e-pTeX: fetch \(no)auto(x)spacing status
%   * e-upTeX: fetch \(enable|disable|force)cjktoken status
%   * TeX Live: pTeX has \showmode, follow showstream.ch r61589
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
@x
